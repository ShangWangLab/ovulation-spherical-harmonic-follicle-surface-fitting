...
def compute_loss(model, sph_coords, rad_grad):
    ...
    total = (WEIGHTS["radius"] * radius_loss
...
SCHEDULE = [
    (0, 0.6, 7),  # Just the radius.
    (2, 0.7, 5),
    (4, 0.5, 3),
    (6, 0.3, 3),
    (8, 0.2, 3),
    (10, 0.1, 10),  # Normally 10, sometimes 9.
]
...
volume_integral = IntegralSurface(DEVICE, N_THETA, N_PHI)

dir_img = ROOT + "follicles/img/"
dir_old_fit = ROOT + "follicles/fit/"
dir_refit = ROOT + "follicles/refit/"
...
for di in TO_PROCESS:
    ...
    for fol in follicles:
        ...
        fol_old_fit_dir = dir_old_fit + fol.label + "/"
        fol_refit_dir = dir_refit + fol.label + "/"
        fol_surf_dir = dir_surf + fol.label + "/"
        os.makedirs(fol_refit_dir, exist_ok=True)
        ...
        model = SphericalSurface(CENTER_INIT, fol.r, L_MAX, False).to(DEVICE)
        for it, volume in enumerate(stack):
            old_state = torch.load(fol_old_fit_dir + f"{it:03d}.pt")
            L_old = 9 if old_state["coeffs"].shape[0] == 100 else 10
            old_model = SphericalSurface(CENTER_INIT, 1, L_old)
            old_model.load_state_dict(old_state)
            old_center = volume_integral.center_of_mass(old_model)
            #print("Center:", old_model.center, "vs. CoM:", old_center)
            for i in range(3):
                model.center.data[i] = old_center[i]
            ...
            torch.save(model.state_dict(), fol_refit_dir + f"{it:03d}.pt")
