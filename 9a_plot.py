...
class AnalysisFollicle:
    def __init__(self, line):
        (scan_id, oocyte_id, name, t_hole, t_release, crop_t, delta_t,
         t_deflate, before_deflate, t_v94, t_v92,
         Kordinal, mouse_num, fol_num) = line[:14]
        self.Kordinal = int(Kordinal)  # Kohei's ordering
    ...
    def pvr1_hole_vis(self):
        # Valid for all 9.
        # 0-14 seconds before a hole is visible
        t0 = self.t_hole - math.ceil(14 / self.delta_t)
        t1 = self.t_hole + 1
        return np.array(self.pvr[t0:t1])

    def pvr_x(self):
        # Valid for all 9.
        # 40-54 seconds before oocyte leaves. This often results in a leaking
        # follicle. Not good.
...
##t0s = []
##for f in anafols:
##    t0 = f.t_deflate - math.ceil(50 / f.delta_t)
##    t1 = f.t_deflate
##    t0s.append(int(t0 + t1)//2)
##    print(t0, t1)
##t1s = []
##for self in anafols:
##    t0 = self.t_release - math.ceil(50 / self.delta_t)
##    t1 = self.t_release - math.floor(40 / self.delta_t)
##    t1s.append(int(t0 + t1)//2)
##    print(f"{t0}:{t1} -> {t1 - t0}")
##print("Time indices:")
##times = dict(zip([f.label for f in anafols], zip(t0s, t1s)))
##print(times)
##
...
##print(t_results)
# t = 2.710716
# p-value = 0.02449674 (2.45%)
# DoF = 8.779482

##deltas = np.array([
##    f.pvr1().mean() - f.pvr0().mean()
##    for f in anafols if f.has_pvr0])
##t_paired = deltas.mean() / (np.sqrt(deltas.var(ddof=1) / deltas.size))
##dof = deltas.size - 1
##print("T (paired) =", t_paired, "dof =", dof)
##t_res_paired = stats.ttest_1samp(deltas, 0, alternative="two-sided")
##print("Paired T-test:")
##print(t_res_paired)
### t = 2.744743
### p-value = 0.03352434110275229 (3.35%)
### DoF = 6
##
##
##
...
##    ax.plot([t_deflate, t_deflate], yrange, "k-")
##    ax.plot([t_hole, t_hole], yrange, "k-")
##    ax.set_title(afol.label)
##    ax.set_xlabel("Time (s)")
##    ax.set_ylabel("Patch Volume Ratio")
##    ax.set_ylim(yrange)
##    ax.grid(True)
##plt.tight_layout()
###plt.suptitle("PVR over time (separate)")
##plt.savefig("../charts/PVR over time separate.png",
##            dpi=DPI, bbox_inches="tight")
##plt.show()
##
##for afol in anafols:
...
anafols.sort(key=lambda f: f.Kordinal)

labels0 = [f.paper_label for f in anafols if f.has_pvr0]
...
plt.bar(labels1, pvr1s, yerr=pvr1errs, align="edge", width=w)
#plt.xticks(rotation=25)
plt.ylabel("Mean Cone Ratio")
plt.ylim([0.8, 1.4])
plt.grid(True)
plt.savefig("../charts/change in mean CR 94-92p - Kohei order.png",
            dpi=DPI, bbox_inches="tight")
plt.show()
